...
## 15. Graphify & Knowledge Management

This repository uses `graphify` to maintain a navigable knowledge graph of the codebase and its foundational documents.

### Using the Graph
- **Interactive Visualization:** Open `graphify-out/graph.html` in a browser.
- **Audit Report:** Read `graphify-out/GRAPH_REPORT.md` for community detection, god nodes, and surprising connections.
- **Querying the Graph:** Use `/graphify query "your question"` to traverse the graph for answers grounded in the codebase.
- **Finding Paths:** Use `/graphify path "Concept A" "Concept B"` to see how ideas are connected.

### Maintenance Workflow
- **After Code/Doc Changes:** Run `/graphify --update` to incrementally re-extract and rebuild the graph.
- **After Significant Refactors:** Run `/graphify` to perform a full re-extraction.
- **Adding Research:** Use `/graphify add <url>` to fetch papers, tweets, or videos and merge them into the knowledge graph.

### Token Efficiency
The graph provides a **~400x token reduction** for high-level architectural questions by summarizing the entire corpus into 27 nodes and 17 edges. Check the graph before reading the full documentation for complex queries.
