...
class TestStateChange:
    def test_to_dict(self):
        sc = StateChange(
            plugin_id="p1", state_name="mode", old_value="idle", new_value="active", timestamp="t"
        )
        d = sc.to_dict()
...
class TestStateStore:
    def test_set_and_get(self, tmp_path):
        store = StateStore(
            plugin_id="p1", state_dir=str(tmp_path / "state"), backup_dir=str(tmp_path / "backup")
        )
    ...
    def test_persistence(self, tmp_path):
        ...
        store1 = StateStore(
            plugin_id="p1", state_dir=state_dir, backup_dir=str(tmp_path / "backup")
        )
